            ...
            <div className="mt-6 flex gap-3">
              {socialLinks.map(({ label, href, symbol }) => (
                <a
                  ...
                  href={href}
                  aria-label={label}
                  className="flex h-9 w-9 items-center justify-center rounded-lg border border-white/10 text-zinc-500 text-sm font-bold transition-colors hover:border-[#E8FF00]/30 hover:text-[#E8FF00]"
                >
                  {symbol}
                </a>
              ))}
          ...
          <div className="flex flex-col items-start justify-between gap-6 sm:flex-row sm:items-center">
            ...
            <form className="flex w-full max-w-sm gap-2" onSubmit={(e) => e.preventDefault()}>
              <input
                ...
                className="flex-1 rounded-lg border border-white/10 bg-white/5 px-4 py-2 text-sm text-white placeholder-zinc-500 outline-none focus:border-[#E8FF00]/50 focus:ring-1 focus:ring-[#E8FF00]/20"
              />
              <button
                type="submit"
                className="rounded-lg bg-[#E8FF00] px-4 py-2 text-sm font-semibold text-black transition-colors hover:bg-[#d4e800]"
              >
                Subscribe
              </button>
            </form>
